# Laboratório 02 — Experimentando os parâmetros do ACO
import random
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
if __name__ == '__main__':
    configuracoes = [
        ('base', 1.0, 2.0, 0.5, 20), ('alpha_baixo', 0.1, 2.0, 0.5, 20),
        ('alpha_alto', 5.0, 2.0, 0.5, 20), ('beta_baixo', 1.0, 0.5, 0.5, 20),
        ('beta_alto', 1.0, 5.0, 0.5, 20), ('evaporacao_lenta', 1.0, 2.0, 0.1, 20),
        ('evaporacao_rapida', 1.0, 2.0, 0.9, 20), ('poucas_formigas', 1.0, 2.0, 0.5, 5),
        ('muitas_formigas', 1.0, 2.0, 0.5, 50)]
    print('========== RESULTADO DOS EXPERIMENTOS ==========')
    for i, (nome, alpha, beta, evap, formigas) in enumerate(configuracoes):
        rota, custo, hist, _ = executar_aco(seed=100+i, num_formigas=formigas, alpha=alpha, beta=beta, taxa_evaporacao=evap)
        print(f'{nome}: formigas={formigas}, alpha={alpha}, beta={beta}, evap={evap}, rota={rota}, custo={custo}')
